...
if ($_GET["robot"] == 'true') {
  $nestabilita['nc'] += ceil($nestabilita['dnc'] / $nestabilita['div']);
  if ($nestabilita['nc'] < 0) {
    $nestabilita['nc'] = 0;
...
if ($nestabilita['vyrazeno'] == 'Y') {
  // jadro bouchlo, reaktor se chladi
  $audio = 4; // alarm sirena
  $lights = 0; // zhasnuto
  $reverse = 1;
  $pulsespeed = 3.8;
  $rotationspeed = 50;
  ...
  $text = '<br/>';
  if ($nestabilita['nc'] > 900) {
    // CELKOVA NESTABILITA je VYSOKA
    ...
    // CELKOVA NESTABILITA je NIZKA (pod 900)
  ...
  if ($nestabilita['dnc'] > 65) {
    // kdyz toho udelaji v jednom okamziku HODNE
    $lights = 2;
...
<?php
  ...
  if ($nestabilita['ending'] > 1) {
    $lights = $nestabilita['ending'] - 2;
  }
  else if ($nestabilita['randomizer'] > 0) {
    $lights = 2;
  }
  if (true || ($nestabilita['lastvideo'] != $video) || ($nestabilita['lastlights'] != $lights)) {
